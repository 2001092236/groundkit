...
- **Поиск тоже с резервом.** SearXNG (свой, безлимитный) → DuckDuckGo → Jina → Exa → Brave.
- **Claude Code CLI как чистая LLM** — для локальных экспериментов и прогонов эталонных вопросов.

Это тонкая обёртка: тяжёлую работу делают [LiteLLM](https://github.com/BerriAI/litellm) и [SearXNG](https://github.com/searxng/searxng).
...
1. **Поиск.** Провайдеры пробуются по очереди; результаты фильтруются по белому списку доменов (с поддоменами) на нашей стороне, поэтому поведение одинаково у всех.
...
| `jina` | опционально | ~20 запросов/мин без ключа | Сразу с текстом страниц |
...
| `search_provider`, `search_errors` | Какой поиск сработал и почему не сработали остальные |
...
pip install -e ".[dev]"
ruff check . && pytest -q                    # 41 юнит-тест, без сети
